...
orig_X_train = np.copy(inception_data_sets.train.x)
Y_train = inception_data_sets.train.labels
Y_test = inception_data_sets.test.labels
...
plt.figure()
iter_f = np.load('output/dogfish_koda_inception_wd-0.001_attack_normal_loss_testidx-all_dogfish_koda_trainidx-[1141]_stepsize-0.005_proj_iter-100.npz')
ftest_pred = iter_f['test_pred'] 
ind_s = [i for (i, x) in enumerate(ftest_pred) if x[0]<0.5]
plt.imshow((np.reshape(data_sets.test.x[ind_s, :], [299, 299, 3]) + 1) / 2, interpolation='none')  
plt.title('%s -> %s' % (
                orig_Y_pred[ind_s, 0][0], # Change label if using 300 test set
                iter_f['test_pred'][ind_s, 0][0]))
plt.axis('off')
plt.savefig("figs/attack-dog-single.png", dpi=300, bbox_inches='tight')


plt.figure()
fig, axs = plt.subplots(1, 3, figsize=(15, 6))
diff = 0.5 - (255 * (0.5 - (((iter_f['poisoned_X_train_image'] - data_sets.train.x[idx_to_poison, :]) + 1) / 2)))
axs[0].imshow((np.reshape(data_sets.train.x[idx_to_poison, :], [299, 299, 3]) + 1) / 2, interpolation='none')  
axs[0].axis('off')
...
print(np.max(np.abs(data_sets.train.x[idx_to_poison, :] - iter_f['poisoned_X_train_image'])) * 255 / 2)
